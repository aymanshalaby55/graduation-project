...
export function ChooseVideosModal({ videos }: ChooseVideosModalProps) {
  ...
  // Initialize selectedVideos from context when component mounts or videos change
  useEffect(() => {
    if (videoAnalysisData?.videos?.length) {
      setSelectedVideos(videoAnalysisData.videos);
    }
  ...
  const handleOpenChange = (open) => {
    setIsOpen(open);
    if (!open) {
      setShowUploader(false); // Reset uploader state when main dialog closes
    }
  };
        ...
        <DialogTrigger asChild>
          ...
          >
            <span className="group-hover/modal-btn:translate-x-40 inline-flex items-center transition duration-500">
              ...
              {videoAnalysisData?.videos?.length > 0
                ? `${videoAnalysisData.videos.length} Videos Selected`
                : "Choose Videos"}
            </span>
              ...
              <div className="space-y-6 p-6">
                ...
                {!videos || videos.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <Button 
                      className="w-full sm:w-auto rounded-lg mt-4"
                      variant="secondary"
